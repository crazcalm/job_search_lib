...
pub fn convert_option_string_to_option_date(string_time: Option<String>) -> Option<DateTime<Local>> {
...
pub fn parse_time(date: &str) -> Result<DateTime<Local>, std::num::ParseIntError> {
    ...
    let date_parts: Vec<u32> = date_time_parts[0].split("-")
        .map(|part|{ part.parse::<u32>().unwrap()})
        .collect();

    let time_parts: Vec<u32> = date_time_parts[1].split(":")
        .map(|part|{ part.parse::<u32>().unwrap()})
        .collect();
